...
class SpellCheckCategoryServiceImplTest {
    ...
    void testRemoveCategoryFromSpellCheck_Success() {
        ...
        Category category = new Category();
        category.setId(2L);
        spellCheck.getCategories().add(category);
    ...
    void testGetSpellChecksByCategory_Success() {
        List<SpellCheckCategory> spellChecks = Arrays.asList(new SpellCheckCategory(), new SpellCheckCategory());
        when(spellCheckCache.get("testCategory")).thenReturn(null);
        when(spellCheckCategoryRepository.findByCategoryName("testCategory")).thenReturn(spellChecks);

        List<SpellCheckCategory> result = spellCheckCategoryService.getSpellChecksByCategory("testCategory");
        assertEquals(2, result.size());
        verify(spellCheckCache).get("testCategory");
        verify(spellCheckCategoryRepository).findByCategoryName("testCategory");
        verify(spellCheckCache).put(eq("testCategory"), eq(spellChecks));
    }
}